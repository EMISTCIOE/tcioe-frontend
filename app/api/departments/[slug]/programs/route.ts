...
// Proxies to: GET /api/v1/public/department-mod/departments/{slug}/programs
export async function GET(request: NextRequest, { params }: { params: { slug: string } }) {
  try {
    ...
    console.error("Department programs API error:", error);
    return NextResponse.json(
      { error: "Failed to fetch department programs", message: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 }
    );
  }
}
