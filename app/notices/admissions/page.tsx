...
export default function AdmissionNoticesPage() {
  ...
  // Fetch admission notices from API
  const fetchAdmissionNotices = async () => {
    ...
    try {
      ...
      // Filter notices for "Admission" category
      const admissionNotices = data.filter((notice: any) => 
        notice.notice_category?.notice_type === "Admission"
      );
      
      setNotices(admissionNotices);
      setNoticesNotFound(admissionNotices.length === 0);
    } catch (error) {
      console.error('Error fetching admission notices:', error);
      setNotices([]);
  ...
  return (
    <AnimatedSection className="py-20 px-4 max-w-6xl mx-auto">
      <div className="text-center mb-10">
        <h1 className="text-4xl font-bold text-primary-blue mb-4">Admission Notices</h1>
        <p className="text-lg text-text-dark">Important notices regarding admissions and enrollment.</p>
      </div>
      ...
      {loading && (
        <div className="py-10">
          ...
          <p className="text-center mt-4 text-gray-600">Loading admission notices...</p>
        </div>
      )}
      ...
      {!loading && noticesNotFound && (
        <div className="text-center py-10 bg-gray-50 rounded-lg border border-gray-200">
          <p className="text-gray-600">No admission notices found at the moment.</p>
        </div>
      )}
